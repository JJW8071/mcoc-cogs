...
class Scheduler:
    ...
    async def _add_event(self, name, command, dest_server, dest_channel,
                         author, timedelta, repeat=False, starttime=None):
    ...
    async def _scheduler_repeat(self, ctx, name, time_interval, 
                #startime=None, *, command):
                *, command):
    ...
    @scheduler.command(pass_context=True, name="enh_repeat")
    async def _scheduler_enh_repeat(self, ctx, name, start_delay, time_interval, 
                *, command):
        """Add a command to run every [time_interval] seconds starting [start_delay] from now.
        ...
        name = name.lower()
        try:
            rs = self._parse_time(time_interval)
            ss = self._parse_time(start_delay)
            log.debug('run command in {}s every {}s'.format(ss, rs))
        except:
            await self.bot.send_cmd_help(ctx)
            return
        if rs < 30:
            await self.bot.reply('yeah I can\'t do that, your time'
                                 ...
                                 ' 30 seconds.')
            return
        log.info('add {} "{}" to {} on {} every {}s starting {}s from now'.format(
            name, command, channel.name, server.name, rs, ss))
        await self._add_event(name, command, server, channel, author, rs, 
                    repeat=True, starttime=ss+int(time.time()))
        await self.bot.say('"{}" will run "{}" every {}s starting {}s from now'.format(
                    name, command, rs, ss))
    ...
    @scheduler.command(pass_context=True, name='next')
    async def _scheduler_next(self, ctx, name):
        """Reports the time until the next scheduled repeat event
        """
        event, name = self.get_event(ctx, name)
        if event is None:
            await self.bot.say(name)
            return
        now = time.time()
        int_del = ceil((now-event['starttime'])/event['timedelta'])
        next_event = event['starttime'] + int_del * event['timedelta'] - now
        msg = []
        for txt, val in zip(('Weeks', 'Days', 'Hours', 'Mins', 'Secs'), 
                    self.format_timedelta(next_event)):
            if val:
                msg.append('{} {}'.format(val, txt))
        await self.bot.say("Time until next '{}' event:  {}".format(name, 
                ', '.join(msg)))

    @scheduler.command(pass_context=True, name='echo')
    async def _scheduler_echo(self, ctx, name):
        """Echos the command on the repeat event
        """
        event, name = self.get_event(ctx, name)
        if event is None:
            await self.bot.say(name)
            return
        prefix = self.bot.settings.get_prefixes(ctx.message.server)[0]
        await self.bot.say('```{}{}```'.format(prefix, event['command']))

    def get_event(self, ctx, name):    
        server = ctx.message.server
        name = name.lower()
        if server.id not in self.events:
            return None, 'No events scheduled for this server.'
        if name not in self.events[server.id]:
            return None, 'That event does not exist on this server.'
        return self.events[server.id][name], name
    ...
    def format_timedelta(delta):
        ...
        mins = int(delta/60)
        delta -= mins * 60
        return (weeks, days, hours, mins, int(delta))
    ...
    def _parse_time(self, timestr):
        ...
        for time in timestr.split():
            ...
            if timespec.lower() not in translate:
                raise ValueError
            timeint = int(time[:-1])
            secs += timeint * translate.get(timespec)
        return secs
