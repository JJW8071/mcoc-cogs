    ...
    async def mastery_info(self, ctx, word: str, rank:int = None):
        ...
        if found:
            ...
            for r in range(1, maxranks):
                mrank = str(r)
                effects = cm[key][mrank]['effects'].split(',')
                em = discord.Embed(color=embedcolor, title=titled.format(r), description = desc.format(effects[e] for e in range(0,len(effects)-1))
                unlock_costs = []
                rankup_costs = []
