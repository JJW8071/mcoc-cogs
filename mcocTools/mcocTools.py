...
class MCOCTools:
    ...
    async def _alliance_popup(self, ctx, *args):
        ...
        alliance = await discord.Client.create_role(self, server=server, name='alliance', permissions=67624001, color=discord.Color.yellow(), hoist=True, mentionable=True)
        summoners = await discord.Client.create_role(self, server=server, name='Summoners', permissions=515137, color=discord.Color.white(), hoist=True, mentionable=True)
    # @checks.admin_or_permissions(manage_server=True, manage_roles=True)
