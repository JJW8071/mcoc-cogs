    ...
    async def imgur_random(self, ctx, *, term: str=None):
        ...
        try:
            ...
            if results:
                ...
                em=discord.Embed(color=discord.Color.gold(),title='',description='')
                em.set_image(url=link)
                em.set_footer(text='imgur',icon_url='http://www.iconsdb.com/icons/preview/white/imgur-xxl.png')
        ...
        async with aiohttp.get(url) as r:
            ...
            if r.status == 200:
                if result["data"]:
                    ...
                    em=discord.Embed(color=discord.Color.gold(),title='',description='')
                    em.set_image(url=result['data'][0]['url'])
                    await self.bot.say(embed=em)
                else:
                    await self.bot.say("No results found.")
            else:
